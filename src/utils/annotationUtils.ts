import type { Annotation, HighlightAnnotation, TextBoxAnnotation, DrawingAnnotation, CommentAnnotation, SignatureAnnotation } from '@/types/annotations';
...
export class AnnotationUtil {
  ...
  static getAnnotationBounds(annotation: Annotation): { x: number; y: number; width: number; height: number } {
    switch (annotation.type) {
      ...
      case 'text-box':
      case 'signature': {
        const typed = annotation as HighlightAnnotation | TextBoxAnnotation | SignatureAnnotation;
        return {
          x: typed.x,
          y: typed.y,
          width: typed.width,
